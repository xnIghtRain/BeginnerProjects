...
namespace T_Rex_Runner
{
    public partial class Form1 : Form
    {
        ...
        private void gameEvent(object sender, EventArgs e)
        {
            ...
            {
                // is X a picture box and it has a tag of obstacles
                if (x is PictureBox && x.Tag == "obstacle")
                {
                    ...
                    {
                        // if the obstacles have gone of the screen
                        // then we respawn it off the far right
                        // in this case we are calculating the form width and a random number btw 200 and 800
        ...
        public void resetGame()
        {
            ...
            force = 12; // set the force to 8
            trex.Top = floor.Top - trex.Height; // also we place the player on top of the picture box
            jumpSpeed = 0; // set jump speed to 0
            ...
            foreach (Control x in this.Controls)
            {
                ...
                if (x is PictureBox && x.Tag == "obstacle")
                {
                    // generate a random number in the position integer btw 600 and 1000
                    int position = rnd.Next(600, 1000);

                    // change the obstacles left position to a random location beginning of the game
